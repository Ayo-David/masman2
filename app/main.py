...
def register(report=None):
    if request.method =='POST':
        ...
        existing_user = user.find_one({'username' : request.form['name']})
        ...
        #gg_parent = generation2.find_one({'user_id' : grand_parent['referral_id']})
        ...
        if existing_referral:
            ...
            #hashpassword = bcrypt.hashpw(request.form['password'].encode('utf-8'), bcrypt.gensalt())
                user.insert({
                    'user_id':user.find().count()+1, #auto increment in the db
            ...
            #report = 'Grand Parent' + str(grand_parent) + 'Referal Parent' + str(referral_parent)
...
def userdashboard():
    ...
    if g.user:                                                                                                                                                             
        ...
        return render_template('referral.html', downline = downline1, user=session['username'])
...
def dropsession():
    session.pop('username', None)
    repor = 'You are Successfully logged out'
