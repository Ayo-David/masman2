...
def reward(myid):
    ...
    if downline1_count < 6:
        ...
        print('You need to refer more people')
...
def userdashboard():
    ...
    if g.user:                                                                                                                                                             
        ...
        # leftjoin here  generation2 and user - to get user details
        downline2_details = generation2.aggregate([
        {
            '$match':{'grand_parent':myid} #get for mydownline only
        },
        ...
        #get count for downline2
        downline2_detailscount = generation2.aggregate([
        {
            '$match':{'grand_parent':myid} #get for mydownline only
        },{'$count': "downline2_count"}
        ])
        #  
        # cursorlist = [a for a in downline2_details]
        downline2_detailscount = list(downline2_detailscount) #From pymongo 4.0, useCursor is no longer available, use list() to convert cursor to a list:

        total_downline2 = downline2_detailscount[0]['downline2_count']
       

        # leftjoin here  generation2 and user - to get user details
        downline3_details = generation3.aggregate([
        {
            '$match':{'gg_parent':myid} #get for mydownline only
        },
        ...
        #get count for downline2
        downline3_detailscount = generation3.aggregate([
        {
            '$match':{'gg_parent':myid} #get for mydownline only
        },{'$count': "downline3_count"}
        ])
        #  
        # cursorlist = [a for a in downline2_details]
        downline3_detailscount = list(downline3_detailscount) #From pymongo 4.0, useCursor is no longer available, use list() to convert cursor to a list:

        total_downline3 = downline3_detailscount[0]['downline3_count']

        # for item in downline2_details:
        #     print(item)
        # #print (status['status']).

        #return status['status']
        return render_template('referral.html', downline = downline1, downline2 = downline2_details, downline3 = downline3_details, user=login_user['name'], myid = myid, mystatus = status, total_downline2 = total_downline2, total_downline3 = total_downline3)
